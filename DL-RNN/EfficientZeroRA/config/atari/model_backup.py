...
class RingAttractorLayer(nn.Module):
    def __init__(self, input_size, output_size, num_neurons, connectivity_strength):
        ...
        input_to_hidden = self.create_connectivity_matrix(input_size-(output_size-1), output_size-1, connectivity_strength)
        hidden_to_hidden = self.create_connectivity_matrix_hidden_state(output_size-1, connectivity_strength)

        self.rnn.weight_ih_l0 = nn.Parameter(torch.Tensor(self.append_row_and_columns(input_to_hidden, int(input_size/output_size), 0.1)), requires_grad=True)
        self.rnn.weight_hh_l0 = nn.Parameter(torch.Tensor(self.append_row_and_columns(hidden_to_hidden, 1, 0.1)))
      
    def append_row_and_columns(self, arr, n, value):
        # Create a new row with the specified value
        new_row = np.full((1, arr.shape[1]), value)
        
        # Append the new row to the original array
        arr = np.vstack([new_row, arr])
        
        # Append n columns with the specified value
        new_columns = np.full((arr.shape[0], n), value)
        arr = np.hstack([new_columns, arr])
        
        print(arr)
        return arr  
    
    def create_connectivity_matrix_hidden_state(self, input_size, connectivity_strength):
        # Create a circular connectivity matrix
        connectivity_matrix = np.zeros((input_size, input_size))
        for i in range(input_size):
            for j in range(input_size):
                distance = min(abs(i - j), input_size - abs(i - j))
                if distance !=0:
                    connectivity_matrix[i, j] = connectivity_strength / (distance*2)
                # if distance <ratio:
                #     connectivity_matrix[i, j] = connectivity_strength 
                if distance == 0:
                    connectivity_matrix[i, j] = connectivity_strength  # Handling zero distance case

        # np.savetxt("./logs/matrix_h_2_h_step_" + str(self.step_RA_log), connectivity_matrix, fmt='%.6f')
        return connectivity_matrix
    
    def create_connectivity_matrix(self, input_size, output_size, connectivity_strength):
        # Create a circular connectivity matrix
        connectivity_matrix = np.zeros((output_size, input_size))
        ...
        for i in range(output_size):
            for j in range(input_size):
                ...
                if distance == 0:
                    connectivity_matrix[i, j] = connectivity_strength  # Handling zero distance case
        # np.savetxt("./logs/matrix_i_2_h_step_" + str(self.step_RA_log), connectivity_matrix, fmt='%.6f')
        return connectivity_matrix
    ...
    def forward(self, x):
        ...
        if self.step_RA_log % 30000==0:
         np.savetxt('/mnt/hum01-home01/ambs/p54278mn/EfficientZero_RA/config/atari/weight_ih_l0_' + str(self.step_RA_log) + '.txt', self.rnn.weight_ih_l0.detach().cpu().numpy(), fmt='%.4f')
         np.savetxt('/mnt/hum01-home01/ambs/p54278mn/EfficientZero_RA/config/atari/weight_hh_l0_' + str(self.step_RA_log) + '.txt', self.rnn.weight_hh_l0.detach().cpu().numpy(), fmt='%.4f')
         
        #     print("Input to hidden weights: ", self.rnn.weight_ih_l0)
        #     print("Hidden to hidden weights: ", self.rnn.weight_hh_l0)
        #     print("RA output: ", ring_attractor_output)
            
            # with open ("/home/marcos/Desktop/Atari_100k/EfficientZero_RA/logs/RA_output_" + str(self.step_RA_log), "w+") as f: f.write(str(ring_attractor_output))
...
class PredictionNetwork(nn.Module):
    def __init__(
        ...
        self.fc_policy = mlp(self.block_output_size_policy, fc_policy_layers, action_space_size*8, init_zero=init_zero, momentum=momentum)

        self.scaling_parameter =  nn.Parameter(torch.Tensor([0.1]))
        self.scaling_parameter_RA =  nn.Parameter(torch.Tensor([0.01]))

        self.RA_layer = nn.Sequential(RingAttractorLayer(action_space_size*8,action_space_size,action_space_size,0.1))
       
    def forward(self, x):

        # self.policy_step_log += 1
        for block in self.resblocks:
            x = block(x)
        ...
        policy = torch.mul(policy, torch.abs(self.scaling_parameter_RA))
        policy = self.RA_layer(policy)
        policy = torch.mul(policy, torch.abs(self.scaling_parameter))

        # if self.policy_step_log % 100000==0:
        #     print("Input scaling: ", self.scaling_parameter_RA)
        #     print("Output scaling: ", self.scaling_parameter_RA)
            # with open ("/home/marcos/Desktop/Atari_100k/EfficientZero_RA/logs/scaling_input_param_" + str(self.policy_step_log), "w+") as f: f.write(str(self.scaling_parameter_RA))
            # with open ("/home/marcos/Desktop/Atari_100k/EfficientZero_RA/logs/scaling_output_param_" + str(self.policy_step_log), "w+") as f: f.write(str(self.scaling_parameter))
